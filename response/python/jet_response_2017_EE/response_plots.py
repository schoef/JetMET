...
for i_absEta_bin, absEta_bin in enumerate( absEta_bins ):
    ...
    for i_genPt_bin, genPt_bin in enumerate( genPt_bins ):
        for i_s, s in enumerate(samples):
            responseShape               = responseShape_1D( s.chain, cut_string( "genPt", genPt_bin )+"&&"+cut_string( "abs(eta)", absEta_bin ))
    ...
    jetResponsePlot = Plot.fromHisto(name = "responseShapes_eta_%3.2f_%3.2f"%absEta_bin, histos = responseShape_histos, texX = "gen jet p_{T}" , texY = "" )
...
for var in ["phEF", "neHEF"]:
    ...
    for i_genPt_bin, genPt_bin in enumerate( genPt_bins ):
        ...
        plotting.draw(jetResponsePlot, plot_directory = os.path.join( plot_directory, args.plot_directory), ratio = {'den':0, 'num':[1,2], 'texY':'ratio'}, logY = False, logX = False,
            yRange=(0, 0.5),
    ...
    for i_absEta_bin, absEta_bin in enumerate( absEta_bins ):
        ...
        plotting.draw(jetResponsePlot, plot_directory = os.path.join( plot_directory, args.plot_directory), ratio = {'den':0, 'num':[1,2], 'texY':'ratio'}, logY = False, logX = True,
            yRange=(0, 0.5),
...
for i_genPt_bin, genPt_bin in enumerate( genPt_bins ):
    ...
    plotting.draw(jetResolutionPlot, plot_directory = os.path.join( plot_directory, args.plot_directory), ratio = {'den':0, 'num':[1,2], 'texY':'ratio'}, logY = False, logX = False,
        yRange=(0, 0.5),
...
for i_absEta_bin, absEta_bin in enumerate( absEta_bins ):
    ...
    plotting.draw(jetResolutionPlot, plot_directory = os.path.join( plot_directory, args.plot_directory), ratio = {'den':0, 'num':[1,2], 'texY':'ratio'}, logY = False, logX = True,
        yRange=(0, 0.5),
